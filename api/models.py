...
def _ensure_three_bins(cur):
    ...
    # ✅ Reasegurar 3 bins correctos CON 1kg
    bins_target = [
        (1, "Metal",    1.0),  # ✅ 1.0 en lugar de 5.0
        (2, "Orgánico", 1.0),  # ✅ 1.0 en lugar de 5.0
        (3, "Resto",    1.0),  # ✅ 1.0 en lugar de 5.0
    ]
...
def init_db():
    ...
    CREATE TABLE IF NOT EXISTS config(
      ...
      threshold_percent INTEGER DEFAULT 50  -- ✅ Cambiado de 80 a 50
    );
    ...
    if fresh:
        # ✅ Carga inicial CON 1kg Y 50%
        cur.executemany(
            "INSERT INTO bins(id,name,capacity_kg) VALUES(?,?,?)",
            [(1,"Metal",1.0),(2,"Orgánico",1.0),(3,"Resto",1.0)]  # ✅ 1.0 en lugar de 5.0
        )
        cur.execute("""
        INSERT INTO config(id,steps_per_mm,v_max_mm_s,a_max_mm_s2,bin_positions_mm,threshold_percent)
        VALUES(1,2.5,120.0,400.0,?,50)  -- ✅ 50 en lugar de 80
        ...
        # ✅ Actualizar capacidad de bins existentes a 1.0kg
        cur.execute("UPDATE bins SET capacity_kg = 1.0 WHERE id IN (1,2,3)")
        
        # ✅ Actualizar threshold en config a 50%
        cur.execute("UPDATE config SET threshold_percent = 50 WHERE id = 1")
        
        cur.execute("""
        INSERT INTO config(id,steps_per_mm,v_max_mm_s,a_max_mm_s2,bin_positions_mm,threshold_percent)
        VALUES(1,2.5,120.0,400.0,?,50)
...
def get_config():
    ...
    if not row:
        return {
            ...
            "threshold_percent": 50  # ✅ Cambiado de 80 a 50
        }
    try:
